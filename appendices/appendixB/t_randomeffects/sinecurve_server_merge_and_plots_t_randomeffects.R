# ------------------------------------------------------------------------------
# sinecurve_server_merge_and_plots_t_randomeffects.R
#
# Merges the per-job files written by the three t-random-effect sine-curve
# simulation scripts and draws Supplementary Figure 2 of Online Appendix B: the
# population-curve panels of manuscript Figure 1 (simultaneous coverage, CI
# width, failure rate) under t-distributed random effects.
#
# Run this after all jobs have finished, from this directory:
#
#   Rscript sinecurve_server_merge_and_plots_t_randomeffects.R
#
# Reads  results/sinecurve_server_t_randomeffects_<method>_job<j>_of<n>.RDS
# Writes results/sinecurve_server_t_randomeffects_<method>_results.RDS   (merged, per method)
#        Supplementary_Figure2.pdf   population curve, all three methods
#
# ------------------------------------------------------------------------------
...
methods <- tibble(method = c("snmmTMB", "assist", "snmmAGQ")) %>%
  mutate(prefix   = paste0("sinecurve_server_t_randomeffects_", method),
...
read_jobs <- function(prefix, jobs_dir) {
  ...
  key <- function(j) paste(j$method, j$njobs, j$iterations, j$seed_base, j$nu_re,
...
# ==============================================================================
# 6. Supplementary Figure 2: population curve, all three methods
# ==============================================================================
...
supplementary_figure2 <- (p_top / p_bottom / p_failure) +
  plot_layout(heights = c(0.4, 0.4, 0.2), guides = "collect") &
  theme(legend.position = "bottom")

ggsave("Supplementary_Figure2.pdf", supplementary_figure2, width = 11, height = 11, units = "in")

message("\nwritten: Supplementary_Figure2.pdf")
