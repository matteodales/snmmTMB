#!/bin/bash
# ---------------------------------------------------------------------------
# sinecurve_server_run_t_randomeffects_snmmAGQ.sh
#
# Launches the sine-curve snmmAGQ simulation with t-distributed random effects
# as NJOBS independent Rscript processes, one per detached `screen` session.
#
#   ./sinecurve_server_run_t_randomeffects_snmmAGQ.sh              # NJOBS jobs, all of them
#   ./sinecurve_server_run_t_randomeffects_snmmAGQ.sh 40           # 40 jobs, all of them
#   ./sinecurve_server_run_t_randomeffects_snmmAGQ.sh 40 3 8 14    # 40 jobs, launch only 3, 8, 14
#
# Run from this directory. Each job writes its own results/ and logs/ file;
# sinecurve_server_merge_and_plots_t_randomeffects.R merges the results afterwards.
#
...
#   Rscript sinecurve_server_simulation_t_randomeffects_snmmAGQ.R <job> <njobs>
#
# Before launching, statmod and numDeriv must be installed:
#   Rscript -e 'install.packages(c("statmod", "numDeriv"), repos = "https://cloud.r-project.org")'
# ---------------------------------------------------------------------------
...
SCRIPT="sinecurve_server_simulation_t_randomeffects_snmmAGQ.R"
PREFIX="sinecurve_t_randomeffects_snmmAGQ"            # screen-session name prefix
CPU_OFFSET=180                                        # job i is pinned to core CPU_OFFSET+i; the three
...
echo "Degenerate:     grep -c 'BOUNDARY' ${LOG_DIR}/${PREFIX}_job*.log"
