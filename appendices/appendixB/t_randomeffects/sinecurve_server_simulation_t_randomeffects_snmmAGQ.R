# ------------------------------------------------------------------------------
# sinecurve_server_simulation_t_randomeffects_snmmAGQ.R
#
# snmmAGQ script for the sine-curve simulation with t-distributed random effects
# of Online Appendix B (Supplementary Figure 2)
#
# Identical to ../../../simulation_sinecurve/sinecurve_server_simulation_snmmAGQ.R
...
#
# with <job> in 1..<njobs>; sinecurve_server_run_t_randomeffects_snmmAGQ.sh launches all of the jobs at once.
# Each job writes only its own results/<output_prefix>_job<j>_of<njobs>.RDS.
#
# The estimator is sourced from ../../../src/snmmAGQ_functions_sinecurve.R. The
# number of interior knots is selected by AIC over nk in 0..6 for every dataset
# (fit_select_nk()).
# ------------------------------------------------------------------------------
...
if (length(args) < 1) {
  stop("Usage: Rscript sinecurve_server_simulation_t_randomeffects_snmmAGQ.R <job> [<njobs>]")
}
...
src_dir <- "../../../src"
results_dir <- "results"
output_prefix <- "sinecurve_server_t_randomeffects_snmmAGQ"
