# ------------------------------------------------------------------------------
# sinecurve_server_merge_and_plots_t_residuals.R
#
# Merges the per-job files written by the three t-residual sine-curve simulation
# scripts and draws Supplementary Figure 1 of Online Appendix B: the population-curve panels
# of manuscript Figure 1 (simultaneous coverage, CI width, failure rate) under
# t-distributed residuals.
#
# Run this after all jobs have finished, from this directory:
#
#   Rscript sinecurve_server_merge_and_plots_t_residuals.R
#
# Reads  results/sinecurve_server_t_residuals_<method>_job<j>_of<n>.RDS
# Writes results/sinecurve_server_t_residuals_<method>_results.RDS   (merged, per method)
#        Supplementary_Figure1.pdf   population curve, all three methods
#
# ------------------------------------------------------------------------------
...
methods <- tibble(method = c("snmmTMB", "assist", "snmmAGQ")) %>%
  mutate(prefix   = paste0("sinecurve_server_t_residuals_", method),
...
read_jobs <- function(prefix, jobs_dir) {
  ...
  key <- function(j) paste(j$method, j$njobs, j$iterations, j$seed_base, j$err_df,
...
# ==============================================================================
# 6. Supplementary Figure 1: population curve, all three methods
# ==============================================================================
...
supplementary_figure1 <- (p_top / p_bottom / p_failure) +
  plot_layout(heights = c(0.4, 0.4, 0.2), guides = "collect") &
  theme(legend.position = "bottom")

ggsave("Supplementary_Figure1.pdf", supplementary_figure1, width = 11, height = 11, units = "in")

message("\nwritten: Supplementary_Figure1.pdf")
