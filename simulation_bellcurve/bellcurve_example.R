...
# bellcurve_example.R
#
# This script runs one replication of the bell-curve simulation of manuscript Section 3,
...
#
# The other "server" scripts in this folder are meant to run this same simulation over 8 settings
# and 500 replications, on a HPC cluster.
#
# RUNTIME: A few seconds on a laptop (changes with sample size), plus a one-off TMB compilation of a few minutes the first time.
#
# Structure:
#   1. Settings
...
#   eps_ij ~ N(0, sigma^2)
#
# b1_i shifts the curve vertically, b2_i shifts it horizontally; the two are
# correlated, with rho = 0.5. The values below are setting 1 of the manuscript
# grid.
# ==============================================================================

n      <- 10                  # subjects
m      <- 10                  # observations per subject
sigma  <- 0.2                 # residual sd
...
# true population curve: at b = 0 the bell is centered at t = 0
mu_true <- alpha + exp(-0.5 * t_grid^2)
...
#
# Cubic B-spline basis with K functions. The spline argument is t_ij - b2_i,
# whose range depends on b2, so the knots are fixed on [0,1] and the argument
# is scaled into that interval inside the likelihood (Section 2.3.1, second
# case): v = (t - b2_i - a) / s with a = min(t) - 3 sd_b2, s = max(t) + 3 sd_b2 - a.
# ==============================================================================
...
# estimated correlation, with its delta-method standard error
rho_row <- which(rownames(summary(rep)) == "rho")
cat("snmmTMB: rho_hat =", round(summary(rep)[rho_row, "Estimate"], 3),
    "(SE", round(summary(rep)[rho_row, "Std. Error"], 3), ")\n")
...
# cubic() expects its argument (including the random effect shift) in [0,1], so t in [-2,2] is mapped to
# t / 10 + 1/2
...
u_obs <- fc$data[[1]]$u        # t_scaled_ij - b2_hat_i: shifted observations
...
# forCI$q[[1]] is the kernel multiplied by d1 (all ones here), so we recover the plain kernel K(u_i, u_j).
K_obs <- fc$q[[1]] / tcrossprod(d1)
K_new <- cubic(u_new, u_obs)   # K(u_new, u_obs), 50 x nobs
...
X     <- cbind(Xt, Xc)        # nobs x (2 + nkeep)
X_new <- cbind(Xt_new, Xc_new)  # 50 x (2 + nkeep)
...
X_pop <- cbind(1, sweep(splineDesign(knotseq, t_grid, 4, outer.ok = TRUE),
                        2, col_means, FUN = "-"))
...
for (k in seq_along(subjects_to_plot)) {
  ...
  b <- b_mat[s, ]
  mu_true_s <- alpha + b["b1"] + exp(-0.5 * (t_grid - b["b2"])^2)
  ...
  b_hat_i <- modes[s, ]
  X_i <- cbind(1, sweep(splineDesign(knotseq, t_grid - b_hat_i[2], 4,
                                     outer.ok = TRUE),
                        2, col_means, FUN = "-"))
