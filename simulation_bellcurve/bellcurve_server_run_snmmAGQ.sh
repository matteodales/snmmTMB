#!/bin/bash
# ---------------------------------------------------------------------------
# bellcurve_server_run_snmmAGQ.sh
#
# Launches the bell-curve snmmAGQ simulation as NJOBS independent Rscript
# processes, one per detached `screen` session.
#
#   ./bellcurve_server_run_snmmAGQ.sh              # NJOBS jobs, all of them
#   ./bellcurve_server_run_snmmAGQ.sh 40           # 40 jobs, all of them
#   ./bellcurve_server_run_snmmAGQ.sh 40 3 8 14    # 40 jobs, launch only 3, 8, 14
#
# Run from this directory. Each job writes its own results/ and logs/ file;
# bellcurve_server_merge_and_plots.R merges the results afterwards.
#
...
SCRIPT="bellcurve_server_simulation_snmmAGQ.R"
PREFIX="bellcurve_snmmAGQ"                            # screen-session name prefix
