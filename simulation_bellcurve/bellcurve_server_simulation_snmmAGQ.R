# ------------------------------------------------------------------------------
# bellcurve_server_simulation_snmmAGQ.R
#
# snmmAGQ script for the bell-curve simulation of manuscript Section 3 (Figure 4)
#
...
#
#   Rscript bellcurve_server_simulation_snmmAGQ.R <job> <njobs>
#
# with <job> in 1..<njobs>; bellcurve_server_run_snmmAGQ.sh launches all of the jobs at once.
# Each job writes only its own results/<output_prefix>_job<j>_of<njobs>.RDS.
#
# The estimator is sourced from ../src/snmmAGQ_functions_bellcurve.R. The number
...
if (length(args) < 1) {
  stop("Usage: Rscript bellcurve_server_simulation_snmmAGQ.R <job> [<njobs>]")
}
...
src_dir <- "../src"
results_dir <- "results"
output_prefix <- "bellcurve_server_snmmAGQ"
...
source(file.path(src_dir, "snmmAGQ_functions_bellcurve.R"))
...
run_replication <- function(setting_idx, iter) {
  ...
  # per-subject responses and covariate values, as bspline.snm() takes them
  ...
  out <- tryCatch({
    ...
    # variance parameters: theta = (sd_b1, sd_b2, sigma, rho)
    true_par <- c(sqrt(diag(Sigma_b)), sigma, rho)
    bias_vec <- true_par - theta
    rho_hat <- theta[4]

    # b1_hat, b2_hat: the posterior modes
    b1_hat <- modes[,1]; b2_hat <- modes[,2]
    ...
    bias_vec <- c(bias_vec, b1_bias, b2_bias, alpha_bias)

    se_b_all <- t(sapply(hessians, function(h) sqrt(diag(solve(h)))))
    se_b1 <- se_b_all[,1]; se_b2 <- se_b_all[,2]
    ...
    X_pop <- cbind(1, sweep(splines::splineDesign(knotseq, t_grid, 4, outer.ok = TRUE), 2, col_means, FUN = "-"))
    ...
    for(subject_index in 1:2){
      ...
      b1s <- b_mat[subject_index,"b1"]; b2s <- b_mat[subject_index,"b2"]
      mu_true_subj <- alpha_true + b1s + exp(-0.5 * (t_grid - b2s)^2)

      # estimated curve and CI
      b_hat_i <- modes[subject_index,]
      X_i <- cbind(1, sweep(splines::splineDesign(knotseq, t_grid - b_hat_i[2], 4, outer.ok=TRUE), 2, col_means, FUN = "-"))
    ...
    list(
      ...
      b1_mse = b1_mse, b2_mse = b2_mse,
      b1_coverage = cover_b1, b2_coverage = cover_b2,
      rho_true = rho,
      rho_hat = rho_hat,
