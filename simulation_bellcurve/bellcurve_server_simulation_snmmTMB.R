# ------------------------------------------------------------------------------
# bellcurve_server_simulation_snmmTMB.R
#
# snmmTMB script for the bell-curve simulation of manuscript Section 3 (Figure 4)
#
...
#
#   Rscript bellcurve_server_compile.R                       # once, first
#   Rscript bellcurve_server_simulation_snmmTMB.R <job> <njobs>
#
# with <job> in 1..<njobs>; bellcurve_server_run_snmmTMB.sh launches all of the jobs at once.
...
if (length(args) < 1) {
  stop("Usage: Rscript bellcurve_server_simulation_snmmTMB.R <job> [<njobs>]")
}
...
settings_to_run <- 1:8
seed_base <- 20260812          # change to generate an independent set of datasets

Nsims <- 10000                 # draws for the simultaneous-band critical value

src_dir <- "../src"
results_dir <- "results"
output_prefix <- "bellcurve_server_snmmTMB"
save_every <- 1L
...
#
# The spline argument t_ij - b2_i has no fixed range, so the knots are placed
# on [0,1] and the likelihood scales the argument into that interval
...
center_grid_range <- c(0, 1)   # on the scaled axis, i.e. the knot range
...
#' @return TRUE if every element of x is within its band (simultaneous
...
#' @param x   vector of true values
#' @param upr,lwr  vectors of upper/lower confidence bounds, same length as x
#' @return proportion of elements of x within their band (pointwise
...
#'
#' @param Sigma covariance matrix from sdreport()
#' @return Sigma, symmetrized; if any eigenvalue is not positive, all
#'   eigenvalues below 1e-10 are raised to 1e-10.
symm_pd <- function(Sigma) {
  Sigma <- (Sigma + t(Sigma)) / 2
  eig <- eigen(Sigma, symmetric = TRUE)
  if (!all(eig$values > 0)) {
    eig_values <- eig$values
    eig_values[eig_values < 1e-10] <- 1e-10
    Sigma <- eig$vectors %*% diag(eig_values) %*% t(eig$vectors)
    Sigma <- (Sigma + t(Sigma)) / 2
  }
  Sigma
}
...
cpp_model <- file.path(src_dir, "snmmTMB_likelihood_bellcurve_simulation.cpp")

#' dyn.load() a TMB model
#'
...
run_replication <- function(setting_idx, iter) {
  ...
  out <- tryCatch({
    ...
    # ------------------------------------------------------------------------
    # Starting values (manuscript Section 2.3.2)
    # ------------------------------------------------------------------------
    Params0 <- list(
    ...
    Data0 <- list(
      ...
      x_grid = as.numeric(seq(0, 1, length.out = ngrid))
    )
    ...
    ## run nlme model with spline_start, with a general (correlated) random-effect covariance
    ...
    # the correlation starts at rho = 0, i.e. transf_rho = atanh(0) = 0
    Params <- list(
      ...
      log_lambda = as.numeric(parList0$log_lambda)   # best point seen by nlminb
    )

    randoms <- c("b1", "b2", "vpos")
    ...
    # rho on its natural scale, from the ADREPORT in the .cpp
    se <- summary(rep)
    rho_row <- which(rownames(se) == "rho")
    rho_hat <- if (length(rho_row)) se[rho_row[1], "Estimate"] else NA_real_
    rho_se  <- if (length(rho_row)) se[rho_row[1], "Std. Error"] else NA_real_
    rho_bias <- rho - rho_hat
    rho_cover <- if (is.na(rho_se) || rho_se == 0) NA
                 else as.numeric(rho >= rho_hat - 1.96 * rho_se &
                                 rho <= rho_hat + 1.96 * rho_se)
    ...
    # ------------------------------------------------------------------------
    mu_true <- alpha_true + exp(-0.5 * t_grid^2) # true population curve

    idx <- grep("h_grid", names(rep$value))
    mu_hat <- rep$value[idx]
    Cov_mu <- rep$cov[idx, idx]

    if (any(is.nan(Cov_mu))) stop("Cov_mu ill-conditioned")

    # symmetric and positive definite
    Cov_mu <- symm_pd(Cov_mu)
    ...
    for (subject_index in 1:2) {
      ...
      mu_true_subj <- alpha_true + b1 + exp(-0.5 * (t_grid - b2)^2)
      ...
      if (any(is.nan(Cov_mu_subj))) {
        message("Setting ", setting_idx, " iteration ", iter,
                " subject ", subject_index, ": Cov_mu ill-conditioned")
        next
      }

      Cov_mu_subj <- symm_pd(Cov_mu_subj)
