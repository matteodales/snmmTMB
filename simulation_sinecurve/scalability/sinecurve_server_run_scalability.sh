#!/bin/bash
# ---------------------------------------------------------------------------
# sinecurve_server_run_scalability.sh
#
# Launches the sine-curve scalability study as NJOBS independent Rscript
# processes, one per detached `screen` session.
#
#   ./sinecurve_server_run_scalability.sh              # NJOBS jobs, all of them
#   ./sinecurve_server_run_scalability.sh 50           # 50 jobs, all of them
#   ./sinecurve_server_run_scalability.sh 50 3 8 14    # 50 jobs, launch only 3, 8, 14
#
# Run from this directory. Each job writes its own results/ and logs/ file;
# sinecurve_scalability_merge_and_plots.R merges the results afterwards.
#
# The timings are the measurement: every job is pinned to one core, runs a
# single-threaded BLAS and is niced. Pick NJOBS so that it divides `iterations`
# in sinecurve_scalability.R (50), so that every job holds the same number of
# replications of every cell, and use the same NJOBS for the whole run.
#
...
#
# Before launching, compile the TMB models in ../../src/ once, from ../:
#   Rscript sinecurve_server_compile.R
# ---------------------------------------------------------------------------
...
NJOBS=50                                              # default number of jobs
SCRIPT="sinecurve_scalability.R"
PREFIX="sinecurve_scal"                               # screen-session name prefix
CPU_OFFSET=200                                        # job i is pinned to core CPU_OFFSET+i; disjoint from
                                                      # the ranges of the three main-simulation launchers
...
for i in $JOBS_TO_RUN; do
  ...
  # single-threaded numerics: the jobs are pinned to one core each, and the
  # timings must not depend on what else is running
...
echo "Failed fits:    grep -c 'failed:' ${LOG_DIR}/${PREFIX}_job*.log"
