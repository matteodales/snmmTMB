# ------------------------------------------------------------------------------
# sinecurve_server_merge_and_plots.R
#
# Merges the per-job files written by the three sine-curve simulation scripts and
# draws Figures 1 and 2 of manuscript Section 3.
#
# Run this after all jobs have finished, from this directory:
#
#   Rscript sinecurve_server_merge_and_plots.R
#
# Reads  results/sinecurve_server_<method>_job<j>_of<n>.RDS
# Writes results/sinecurve_server_<method>_results.RDS   (merged, per method)
#        Figure1.pdf   population curve, all three methods
#        Figure2.pdf   subject curves, snmmTMB and snmmAGQ
#
# ------------------------------------------------------------------------------
...
methods <- tibble(method = c("snmmTMB", "assist", "snmmAGQ")) %>%
  mutate(prefix   = paste0("sinecurve_server_", method),
...
build_results <- function(records, meta) {
  ...
  report <- tibble(
    ...
    D_idx = param_grid$D_idx[settings],
...
# display order of the 16 settings, as used in the manuscript figures
new_order <- c(13, 5, 9, 1, 15, 7, 11, 3, 14, 6, 10, 2, 16, 8, 12, 4)
...
results_to_long <- function(results, method) {
  scalar_fields <- c(
    ...
    "b1_mse", "b2_mse", "b3_mse",
    "b1_coverage", "b2_coverage", "b3_coverage",
    "time"
  )
...
plot_boxplot_by_setting <- function(df, value_col, ylab, ylim = NULL, log_y = FALSE,
  ...
  p <- ggplot(df, aes(x = xpos, y = .data[[value_col]], fill = method, group = interaction(xpos, method))) +
    ...
    # 16 settings x 3 methods the dots swamped the boxes
...
# ==============================================================================
# 6. Figure 1: population curve, all three methods
# ==============================================================================
...
p_bottom <- plot_boxplot_by_setting(df_all, "simultaneous_CI_length", "CI width",
                                    ylim = c(0.25, 6), log_y = TRUE,
                                    breaks = c(0.25, 0.5, 1, 2, 3, 4, 5, 6)) +
  guides(fill = "none")
p_failure <- plot_failure_rate(pop_summary) +
  guides(fill = "none")

figure1 <- (p_top / p_bottom / p_failure) +
  plot_layout(heights = c(0.4, 0.4, 0.2), guides = "collect") &
  theme(legend.position = "bottom")

ggsave("Figure1.pdf", figure1, width = 11, height = 11, units = "in")

# ==============================================================================
# 7. Figure 2: subject curves for snmmTMB and snmmAGQ
# ==============================================================================

df_subject <- df_all %>%
  filter(method %in% c("snmmTMB", "snmmAGQ")) %>%
  mutate(method = fct_drop(method))

subject_summary <- summarize_coverage(df_subject,
                                      "subject_pointwise_coverage", "subject_simultaneous_coverage",
                                      "subject_pointwise_CI_length", "subject_simultaneous_CI_length")

p_top_subj <- plot_coverage(subject_summary)
p_bottom_subj <- plot_boxplot_by_setting(df_subject, "subject_simultaneous_CI_length",
                                         "CI width", ylim = c(0.5, 5.5)) +
  guides(fill = "none")

figure2 <- (p_top_subj / p_bottom_subj) +
  plot_layout(heights = c(0.5, 0.5), guides = "collect") &
  theme(legend.position = "bottom")

ggsave("Figure2.pdf", figure2, width = 11, height = 7, units = "in")

message("\nwritten: Figure1.pdf, Figure2.pdf")
