# ------------------------------------------------------------------------------
# sinecurve_server_simulation_snmmTMB.R
#
# snmmTMB script for the sine-curve simulation of manuscript Section 3 (Figures 1 and 2)
#
...
#
# with <job> in 1..<njobs>; sinecurve_server_run_snmmTMB.sh launches all of the jobs at once.
...
if (length(args) < 1) {
  stop("Usage: Rscript sinecurve_server_simulation_snmmTMB.R <job> [<njobs>]")
}
...
src_dir <- "../src"           
results_dir <- "results"
output_prefix <- "sinecurve_server_snmmTMB"
save_every <- 1L
