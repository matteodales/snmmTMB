// snmmTMB_likelihood_sinecurve_simulation.cpp
// ---------------------------------------------------------------------------
// SNMM likelihood for the sine-curve simulation of manuscript Section 3.
//
// Model:
//   y_ij = alpha + b1_i + exp(b2_i) * f(t_ij - expit(b3_i)) + eps_ij
//   b_i = (b1_i, b2_i, b3_i) ~ N(0, D),  D = diag(sd_b1^2, sd_b2^2, sd_b3^2)
//   eps_ij ~ N(0, sigma^2)
//
// b1_i is a subject vertical shift, b2_i a log-amplitude scaling, and b3_i a
// phase shift entering through expit(b3_i) = 1 / (1 + exp(-b3_i)).
//
// Since t_ij is in [0,1] and expit(b3_i) in (0,1), the spline argument lies in
// [-1,1] whatever b3 is, so the knots can be fixed on that interval once and
// held there throughout estimation (Section 2.3.1, first case).
//
...
//   b1, b2, b3               per-subject random effects (vertical shift,
//                            log-amplitude, logit-phase)
//   log_sd_b1/b2/b3          log standard deviations of b1/b2/b3
...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0;i<N;i++){
    int g = group(i) - 1;
    Type phase = Type(1.0) / ( Type(1.0) + exp( - b3(g) ) );
    u(i) = t(i) - phase;
  }
